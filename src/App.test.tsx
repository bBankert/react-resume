...
describe("App data fetching tests", () => {
  ...
  it("Requests the data json file", async () => {
    return storeRef.store
      .dispatch<any>(informationApi.endpoints.getInformation.initiate(null))
      .then(() => {
  ...
  it("Returns a successful response and renders the footer", async () => {
    ...
    const emailLink = links.filter(
      (link) =>
        link.textContent === "Email me" &&
        link.href ===
          encodeURI("mailto:email@test.com?subject=Resume Site Contact")
    );
    expect(emailLink.length).toBe(1);
    expect(links.filter((link) => link.href === "tel:1234567890").length).toBe(
      1
    );
  });
});
